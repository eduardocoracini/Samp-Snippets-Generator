...
namespace nSnippet
{
    public class cSnippet
    {
        public void GenerateSnippets(string path, string[] directives, string saveTo, bool generateDictionay)
        {
            ...
            string snippetsFile = "snippets.sublime-completions";
            string dictionaryFile = "snippets_dictionary.txt";
            string errorFile = "snippets_error.txt";

            snippets.Append(
                "{" + Environment.NewLine +
                "   \"scope\": \"source.pawn\"," + Environment.NewLine +
                "   \"completions\": [" + Environment.NewLine);
                ...
                foreach (string line in lines)
                {
                    ...
                    if (diretiveName != string.Empty)
                    {
                        ...
                        if (functionParams != string.Empty)
                            snippets.Append("       { \"trigger\": \"" + diretiveName + "\", \"contents\": \"" + diretiveName + "(" + functionParams + ")$0\" }," + Environment.NewLine);
                        else
                            snippets.Append("       { \"trigger\": \"" + diretiveName + "\", \"contents\": \"" + diretiveName + "\" }," + Environment.NewLine);
